    ...
    if($isAssoc == true){
      ...
      if($type == 'config'){
        ...
        foreach($options as $opt){
          $chk = $opt;
          $ipt = $opt;
          if($dtmSaved && $dtmSaved['include-'.$opt['name']]){
        ...
        foreach($options as $opt){
          if($dtmSaved && $dtmSaved['disable-'.$opt['name']]){
